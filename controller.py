...
class MainWindow_controller(QtWidgets.QMainWindow):
    ...
    def main_run(self):
        ...
        while self.time_flag==True :
            print("self.flag_sqlinit:",self.flag_sqlinit)
            if self.flag_sqlinit == False:
                ###### SQL #########
                connection = mysql.connector.connect(host='localhost',
                                                    port='3306',
                                                    user='root',
            ...
            if ((self.mode_x != None)and(self.handmode != None)):
                ...
                nonum = int(nonum)
                # print('nonum',nonum)
                print("一開始初始化 score2:   ",score2)
                ####################
                account = self.account
                  
                self.frame,self.frame12,self.time_flag,self.alltime,self.score2,self.df,self.flag_sqlinit = main2.main_strat(account,score2,cnum,nonum,f,self.mode_x,self.handmode,time_flag,flag,vc,timer1,start_time,start_timeall)# main2.strt(self.mode_x,self.handmode)
                f=True
